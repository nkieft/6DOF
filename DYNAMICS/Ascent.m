function dSdt = Ascent(t, s, V_data, L_data, Const, F_T_data, C_b_data, C_c_data, Cp_data, W_h, atm_data)
%ASCENT is an ode45 integratable function that calculates the vehicles
%state throughout ascent, and if ballistic recovery is selected, descent.
...
[C_l, C_m, C_n] = MomentForcingCoefficients(C_Y_D, C_N_D, beta, alpha_p, F_cant, r_v, ymac, L_N, C_r, L_v, CG, P, U);
...
M_F = Moments(C_l, C_m, C_n, C_l_p, C_m_q, C_n_r, q, A_ref, r_v*2, onRail, F_T, L_v, CG);
...
%% Assign Derivatives
%----------------------------------------------------------------------
%See state variable extraction for state vector assignments
dSdt = zeros(24, 1);
dSdt(1) = U_dot;
dSdt(2) = V_dot;
dSdt(3) = W_dot;
...
%Useful breakpoint spot for troubleshooting :)
if(~onRail)
